...
export default function ResumeForm({ templateId, resumeId }: ResumeFormProps) {
  const { data, setData } = useResume();
  const [loading, setLoading] = useState(false);
  const [allowedSections, setAllowedSections] = useState<string[]>([]); // 🆕 dynamic sections

  useEffect(() => {
    // 🧩 Load allowed sections from template
  ...
  return (
    <div className="p-4 border rounded-lg bg-white shadow space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <select
            ...
            onChange={(e) =>
              setData({
                ...data,
                fonts: { ...data.fonts, header: e.target.value },
              })
            }
            className="border p-2 rounded text-gray-900"
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <select
            ...
            onChange={(e) =>
              setData({
                ...data,
                fonts: { ...data.fonts, text: e.target.value },
              })
            }
            className="border p-2 rounded text-gray-900"
          >
      ...
      {/* 👇 Always show name/email/phone (mandatory) */}
      ...
      {allowedSections.includes("skills") && (
        <>
          <h3 className="font-semibold text-gray-900">Skills</h3>
          <input
            type="text"
      ...
      {allowedSections.includes("education") && (
        <>
          <h3 className="font-semibold text-gray-900">Education</h3>
          {data.education?.map((edu, idx) => (
            <div key={idx} className="space-y-2 border p-2 rounded">
      ...
      {allowedSections.includes("experience") && (
        <>
          <h3 className="font-semibold text-gray-900">Experience</h3>
          {data.experience?.map((exp, idx) => (
            <div key={idx} className="space-y-2 border p-2 rounded">
      ...
      {allowedSections.includes("projects") && (
        <>
      ...
      {allowedSections.includes("certifications") && (
        <>
      ...
      {allowedSections.includes("languages") && (
        <>
      ...
      {allowedSections.includes("hobbies") && (
        <>
